...
const labels = files
  .map((f, i) => {
    ...
    const y = Math.floor(i / cols) * tileH + tileH - 8;
    const txt = f.replace('.webp', '');
    return `<text x="${x}" y="${y}" font-family="monospace" font-size="14" fill="white" stroke="black" stroke-width="0.5">${txt}</text>`;
  })
  .join('\n');
...
await sharp({ create: { width: W, height: H, channels: 4, background: { r: 30, g: 30, b: 30, alpha: 1 } } })
  ...
  .png()
  .toFile(`/tmp/contact-${deck}-big.png`);

console.log(`saved /tmp/contact-${deck}-big.png with ${files.length} cards (${cols}x${rows})  ${W}x${H}`);
